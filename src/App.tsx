          ...
          {steps[tabIndex].map((step, idx) => {
            return (
              ...
              >
                <div>{step.text}</div>
                {(() => {
                  const evidence = stepImages[idx];
                  if (!evidence) return null;
                  const urls = Array.isArray(evidence) ? evidence : [evidence];
                  return (
                    <div className="pt-1 flex flex-col gap-2">
                      {urls.map((src, imgIdx) => (
                        <img
                          key={imgIdx}
                          src={src}
                          alt={`Minh chứng bước ${idx + 1}${urls.length > 1 ? ` (${imgIdx + 1})` : ''}`}
                          className="w-full max-w-lg rounded-lg border border-slate-200/60 shadow-xs cursor-zoom-in hover:opacity-90 hover:shadow-sm transition-all object-contain bg-slate-50"
